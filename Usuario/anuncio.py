...
class Anuncio(ft.UserControl):
    ...
    def lista(self, profesion):
        ...
        try:
                    ...
                    conn.commit()
                    usere = (("%"+profesion+"%"),)
                    cursor.execute("SELECT User FROM anuncio WHERE Profesion LIKE ?;", usere)
                    resultado =cursor.fetchall()
                    for resultad in resultado:
                        fotos = ("Usuario/fotos/pfp"+ str(l)+".jpg")
                        l+=1
                        cursor.execute("SELECT Usuario, Nombre, Precio, Imagen from trabajador where Usuario=? ", resultad)
                        r =cursor.fetchone()
                        use = ("@"+r[0])
                        nom = r[1]
                        prec= (str(r[2])+"/h")
    ...
    def enviarsol(self, user2, trabajo):
        ...
        try:
                ...
                sql = "INSERT INTO solicitud (Usuario_Recibe, Usuario_Envia, trabajo) VALUES (?, ?, ?);"
