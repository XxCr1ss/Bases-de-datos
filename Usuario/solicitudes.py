...
class solicitudes(ft.UserControl):
    ...
    def lista(self):
        ...
        try:
                        cursor = conn.cursor()
                        cursor.execute("SELECT ID, Usuario_Recibe, Usuario_Envia, estado, trabajo from solicitud where Usuario_Envia=? ", (self.yo,))
                        ...
                        for ID, usuarioR, UsuarioE, estado, trabajo in resultado:
                            ...
                            if(estado=='Enviado'):
                                print("Sí entró")
                                cursor.execute("SELECT Imagen, Precio from trabajador where Usuario=? ", (usuarioR,))
                                e =cursor.fetchone()    
                                img = e[0]
    ...
    def pagarsol(self, ID, precio, cal):
        ...
        try:
                cursor = conn.cursor()
                # Ejemplo de inserción de datos en una tabla llamada 'Personas'
                sql = "UPDATE solicitud SET estado = 'Pagado' WHERE ID = ?;"
                cursor.execute("INSERT INTO pagos (solicitud_ID, pago, Puntuacion) VALUES (?, ?, ?);", (ID, precio, cal))
                cursor.execute(sql, (ID,))
            # Confirmar la transacción
                conn.commit()
                self.princ.controls.clear()
