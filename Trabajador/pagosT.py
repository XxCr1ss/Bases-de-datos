...
class pagosT(ft.UserControl):
    ...
    def lista(self):
        ...
        try:
                        cursor = conn.cursor()
                        cursor.execute("SELECT ID,  Usuario_Envia, estado from solicitud where Usuario_Recibe=?", (self.yo,))
                        resultado =cursor.fetchall()
                        for ID, UsuarioE, estado in resultado:
                            l+=1
                            if(estado=='Pagado' or estado=='Historial'):
                                cursor.execute("SELECT pago from pagos where solicitud_ID=? ", (ID,))
                                e =cursor.fetchone()  
                                ...
                                row2 = ft.Row([p])
                                user = ft.Text(value=("Pagado por: @"+UsuarioE), size=18, color= ft.colors.BLACK)
                                divider = ft.Divider()
    ...
    def borrarcom(self, ID):
        ...
        try:
                ...
                sql = "DELETE FROM pagos WHERE solicitud_ID = ?;"
