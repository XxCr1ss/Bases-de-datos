...
class solicitud(ft.UserControl):
    ...
    def lista(self):
        ...
        try:
                with conn.cursor() as cursor:
                        cursor.execute("SELECT ID, Usuario_Recibe, Usuario_Envia, estado, trabajo from `art`.`solicitud` where Usuario_Recibe=%s ", self.yo)
                        print(self.yo)
                        ...
                        for ID, usuarioR, UsuarioE, estado, trabajo in resultado:
                            ...
                            l+=1
                            if(estado=='Enviado'):
                                cursor.execute("SELECT Foto, Direccion from `art`.`usuario` where User=%s ", UsuarioE)
                                e =cursor.fetchone()    
                                img = e[0]
    ...
    def aceptarsol(self, ID):
        ...
        try:
            with conn.cursor() as cursor:
                ...
                sql = "UPDATE `art`.`solicitud` SET `estado` = 'Aceptado' WHERE (`ID` = %s);"
    ...
    def borrarsol(self, ID):
        ...
        try:
            with conn.cursor() as cursor:
                # Ejemplo de inserción de datos en una tabla llamada 'Personas'
                sql = "UPDATE `art`.`solicitud` SET `estado` = 'Borrada' WHERE (`ID` = %s);"
                cursor.execute(sql, ID)
            # Confirmar la transacción
                conn.commit()
                self.princ.controls.clear()
    ...
    def terminarsol(self, ID):
        ...
        try:
            with conn.cursor() as cursor:
                # Ejemplo de inserción de datos en una tabla llamada 'Personas'
                sql = "UPDATE `art`.`solicitud` SET `estado` = 'Terminado' WHERE (`ID` = %s);"
                cursor.execute(sql, ID)
            # Confirmar la transacción
                conn.commit()
                self.princ.controls.clear()
